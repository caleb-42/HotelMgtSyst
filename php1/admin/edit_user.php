...
if ($old_user_name != $user_name) {
	$duplicate_check_query = "SELECT * FROM admin_users WHERE role = '$role' AND user_name = '$user_name'";
...
if ($password) {
	$update_user_query = "UPDATE admin_users SET user = '$user', role = '$role', user_name = '$user_name', password = '$password' WHERE id = $id";
} else {
	$update_user_query = "UPDATE admin_users SET user = '$user', role = '$role', user_name = '$user_name' WHERE id = $id";
}
