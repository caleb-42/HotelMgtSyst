...
$stmt = $conn->prepare("SELECT password, role FROM account_users WHERE user_name=?");
...
if (!verify($password, $hashedPassword)) {
	...
	if (mysqli_num_rows($sql_log_out_result) > 0) {
		$update_log_out = "UPDATE account_sessions SET logged_on_state = 'TERMINATED' WHERE user_name= '$username' AND role = '$role' AND logged_on_state = 'LOGGED IN'";

		$update_log_out_result = mysqli_query($dbConn, $update_log_out);
	}
	$sql_insert_login = "INSERT INTO account_sessions (user_name, role, logged_on_state) VALUES ('$username', '$role', 'LOGGED IN')";
